...
class TestServeur(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        (cls.www / "telechargements" / "MasterMix-2.18-Setup-x64.exe").write_bytes(b"MZ" + b"\0" * 3000)
    ...
    def test_statique_et_telechargement(self):
        ...
        r = self.c.req("HEAD", "/telechargements/MasterMix-2.18-Setup-x64.exe")
        self.assertEqual(r.status, 200)
        self.assertEqual(r.getheader("Content-Length"), "3002")
        self.assertIn("attachment", r.getheader("Content-Disposition"))
        r = self.c.req("GET", "/telechargements/MasterMix-2.18-Setup-x64.exe", entetes={"Range": "bytes=0-1"})
